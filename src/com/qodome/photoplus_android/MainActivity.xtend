...
@AndroidActivity(R.layout.activity_main) class MainActivity {
	...
	override cutPhoto(View v) { 
		cropImageView.queryCoordinate()
		var croppedImage = cropImageView.getCroppedImage()
        ...
        for (var i = 0; i < 3; i++) {
        	for (var j = 0; j < 3; j++) {
        		Log.i(getString(R.string.LOGTAG), "i: " + i + " j: " + j)
        		var fn = new File(folderName + "test" + i + j + ".png")
				if (!fn.exists()) {
					fn.createNewFile()
				}
        		var re = new Rect((i * width), (j * height), ((i + 1) * width), ((j + 1) * height))
	...
	override onActivityResult(int requestCode, int resultCode, Intent data) {
    	if (requestCode == 42 && resultCode == RESULT_OK) {
        	cropImageView.setImageBitmap(MediaStore.Images.Media.getBitmap(this.getContentResolver(), data.getData()))
    	}
	}
